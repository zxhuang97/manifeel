    ...
    with threadpool_limits(1):
        ...
        class RealImageDataset(BaseImageDataset):
            ...
            def get_normalizer(self, **kwargs) -> LinearNormalizer:
                ...
                for key in self.lowdim_keys:
                    normalizer[key] = SingleFieldLinearNormalizer.create_fit(self.replay_buffer[key])
                    normalizer['robot_eef_quat'] = SingleFieldLinearNormalizer.create_fit(
                        self.replay_buffer['robot_eef_quat'])
                    normalizer['robot_eef_pos'] = SingleFieldLinearNormalizer.create_fit(
                        self.replay_buffer['robot_eef_pos'])
        ...
        def test():
            import os
            zarr_path = os.path.expanduser('data/can/0327_can_50/rgb/8eb7ffc9dd553b7d2ccf5169577b2c2f.zarr.zip')
            dataset = RealImageDataset(dataset_path, horizon=16)
...
def test():
    import os
    zarr_path = os.path.expanduser('data/can/0327_can_50/rgb/8eb7ffc9dd553b7d2ccf5169577b2c2f.zarr.zip')
    dataset = RealImageDataset(dataset_path=zarr_path, horizon=16)

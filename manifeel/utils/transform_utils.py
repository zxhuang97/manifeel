...
def quaternion_to_matrix(quat: torch.Tensor) -> torch.Tensor:
    """Converts a batch of quaternions (B, 4) to rotation matrices (B, 3, 3)."""
    i, j, k, w = quat.unbind(-1)
    two_s = 2.0 / (quat.pow(2).sum(dim=-1, keepdim=True))
    R = torch.empty(quat.shape[0], 3, 3, device=quat.device)  # (B, 3, 3)
    R[:, 0, 0] = 1 - two_s.squeeze(-1) * (j * j + k * k)
    R[:, 0, 1] = two_s.squeeze(-1) * (i * j - k * w)
    R[:, 0, 2] = two_s.squeeze(-1) * (i * k + j * w)
    R[:, 1, 0] = two_s.squeeze(-1) * (i * j + k * w)
    R[:, 1, 1] = 1 - two_s.squeeze(-1) * (i * i + k * k)
    R[:, 1, 2] = two_s.squeeze(-1) * (j * k - i * w)
    R[:, 2, 0] = two_s.squeeze(-1) * (i * k - j * w)
    R[:, 2, 1] = two_s.squeeze(-1) * (j * k + i * w)
    R[:, 2, 2] = 1 - two_s.squeeze(-1) * (i * i + j * j)
    return R

def quaternion_to_sixd(quat: torch.Tensor) -> torch.Tensor:
    """Converts quaternions (B, 4) to 6D rotation representation (B, 6)."""
    quat = torch.nn.functional.normalize(quat, p=2, dim=-1)
    rot_matrices = quaternion_to_matrix(quat)
    sixd_repr = rot_matrices[:, :2, :].reshape(-1, 6)
    return sixd_repr
